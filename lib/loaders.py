...
class Loader(ABC):
    ...
    def load(self, train_test_split_internal=None):
        X_train, y_train, X_test, y_test = self._load_preprocessed_data(train_test_split_internal)
        return X_train, y_train, X_test, y_test

    @abstractmethod
    def _load_preprocessed_data(self, train_test_split_internal):
        pass
...
class LibsvmLoader(Loader):
    ...
    def _load_preprocessed_data(self, train_test_split_internal):
        if self._path_of_test is None:
            ...
            if train_test_split_internal is None:
                X_train, y_train = X[ind[200:]], y[ind[200:]]
                X_test, y_test = X[ind[:200]], y[ind[:200]]
            else:
                X_train, y_train = X[ind[:int(len(ind) * train_test_split_internal)]], \
                                   y[ind[:int(len(ind) * train_test_split_internal)]]
                X_test, y_test = X[ind[int(len(ind) * train_test_split_internal):]], \
                                 y[ind[int(len(ind) * train_test_split_internal):]]
        else:
...
class MNISTDataLoader(data.Dataset):
    def __init__(self, root_path, data_name, label_name):
        data_name = os.path.join(root_path, data_name)
        label_name = os.path.join(root_path, label_name)
        with gzip.open(label_name, 'rb') as labelpath:
            self.labels = np.frombuffer(labelpath.read(), np.uint8, offset=8)
        self.data_len = len(self.labels)
        with gzip.open(data_name, 'rb') as imgpath:
            self.dataset = np.frombuffer(imgpath.read(), np.uint8, offset=16).reshape(self.data_len, 784)
...
class LoaderFactory:
    ...
    def create(self, name, root, random=True, seed=None, partial=False,
        ...
        if name == 'gisette':
            ...
            loader = YangMNISTLoader(784, seed)
        elif name == 'yang-fmnist':
            loader = YangFMNISTLoader(784, seed)
        elif name == 'mnist':
