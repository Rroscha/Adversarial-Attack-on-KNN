...
class AttackModel:
    def __init__(self, dknn, gep, log):
        self.dknn = dknn
    ...
    def __call__(self, x_orig, label, norm, guide_layer=['relu1'], m=100,
                 ...
                 check_adv_steps=50, verbose=True):
        """
        Parameters
        ----------
        dknn : DKNN object
        ...
        self.coeff = torch.zeros((x_orig.size(0), m), device=self.device)  # wi?
        self.coeff[:, :m // 2] -= 1
        self.coeff[:, m // 2:] += 1

        # variables representing inputs in attack space will be prefixed with z
        z_orig  = AttackModel.to_attack_space(x_orig)
        x_recon = AttackModel.to_model_space(z_orig)
        ...
        if init_mode == 1:
            ...
            if verbose:
                ...
                with torch.no_grad():
                    ...
                    z_init = AttackModel.to_attack_space(x_init.to(x_orig.device)) - z_orig
        ...
        for binary_search_step in range(binary_search_steps):
            ...
            if not random_start:
                ...
                rand = np.random.randn(*input_shape) * 1e-2   # *input_shape 解压参数列表
            ...
            for iteration in range(max_iterations):
                optimizer.zero_grad()
                x = AttackModel.to_model_space(z_orig + z_delta)
                ...
                with torch.no_grad():
                    if iteration % thres_steps == 0:
                        self.thres = []
                        thres = self.dknn.get_neighbors(x)  # eta
                        for i in range(len(self.layers)):
                            t = torch.tensor(thres[i][0][:, -1]).to(self.device).unsqueeze(-1) # 就是在指定的位置插入一个维度 100 -> 100,1
                            self.thres.append(t)
                ...
                if time.time() - start_time > self.gep['time_limit'] * len(label):
    ...
    def loss_function(self, x, reps, const, x_recon, norm):
        ...
        for l, layer in enumerate(self.layers):
            ...
            adv_loss[:, l] = torch.sigmoid(
                self.coeff.to(self.device) * fx + DELTA).sum(1)
